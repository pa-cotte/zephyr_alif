...
#ifdef CONFIG_WIFI_WINC1500_SOCKET_OFFLOAD
#include "wifi_winc1500_socket_offload.h"
#endif

/* Socket API declarations are now in winc1500_socket.c */
/* Common socket functions are declared in winc1500_socket.h */

/* HAL API functions declared in winc1500_socket.c */
...
extern void registerSocketCallback(tpfAppSocketCb socket_cb, tpfAppResolveCb resolve_cb);
 ...
 */
static int winc1500_get(sa_family_t family, enum net_sock_type type, enum net_ip_protocol ip_proto,
			struct net_context **context)
{
	int sock;

	sock = __winc1500_socket_new(family, type, ip_proto, context);
	if (sock < 0) {
		return sock;
	}
 ...
 */
static int winc1500_bind(struct net_context *context, const struct sockaddr *addr,
			 socklen_t addrlen)
{
	SOCKET socket = (int)context->offload_context;

	return __winc1500_bind(socket, addr, addrlen);
}
...
static int winc1500_listen(struct net_context *context, int backlog)
{
	...
	return __winc1500_listen(socket, backlog);
}
 ...
 * to a peer host.
 */
static int winc1500_connect(struct net_context *context, const struct sockaddr *addr,
			    ...
			    void *user_data)
{
	...
	ret = __winc1500_connect(socket, addr, addrlen);
	if (ret) {
		return ret;
	}

	if (timeout != 0 && k_sem_take(&w1500_data.socket_data[socket].wait_sem, K_MSEC(timeout))) {
		return -ETIMEDOUT;
	}
 ...
 * being established.
 */
static int winc1500_accept(struct net_context *context, net_tcp_accept_cb_t cb, int32_t timeout,
			   void *user_data)
{
	SOCKET socket = (int)context->offload_context;
	int ret;
	...
	ret = __winc1500_accept(socket);
	if (ret) {
		return ret;
	}

	if (timeout) {
		if (k_sem_take(&w1500_data.socket_data[socket].wait_sem, K_MSEC(timeout))) {
			return -ETIMEDOUT;
		}
	}
 ...
 */
static int winc1500_send(struct net_pkt *pkt, net_context_send_cb_t cb, int32_t timeout,
			 void *user_data)
{
	...
	ret = __winc1500_send(socket, buf->data, buf->len, 0);
	if (ret < 0) {
		goto out;
	}
 ...
 * This function is called when user wants to send data to peer host.
 */
static int winc1500_sendto(struct net_pkt *pkt, const struct sockaddr *dst_addr, socklen_t addrlen,
			   net_context_send_cb_t cb, int32_t timeout, void *user_data)
{
	...
	ret = __winc1500_sendto(socket, buf->data, buf->len, 0, dst_addr, addrlen);
	if (ret < 0) {
		goto out;
	}
...
static int prepare_pkt(struct socket_data *sock_data)
{
	...
	sock_data->rx_pkt = net_pkt_rx_alloc_on_iface(w1500_data.iface, K_NO_WAIT);
	if (!sock_data->rx_pkt) {
 ...
 * host.
 */
static int winc1500_recv(struct net_context *context, net_context_recv_cb_t cb, int32_t timeout,
			 void *user_data)
{
	SOCKET socket = (int)context->offload_context;
	int ret;
	...
	ret = __winc1500_recv(socket, w1500_data.socket_data[socket].pkt_buf->data,
...
static void handle_wifi_dhcp_conf(void *pvMsg)
{
	...
	LOG_DBG("Wi-Fi connected, IP is %u.%u.%u.%u", pu8IPAddress[0], pu8IPAddress[1],
...
static void handle_scan_result(void *pvMsg)
{
	...
	if (pstrScanResult->u8AuthType == M2M_WIFI_SEC_OPEN) {
		...
		LOG_DBG("Security %u not supported", pstrScanResult->u8AuthType);
		goto out;
	}
...
static void handle_scan_done(void *pvMsg)
{
	...
	if (pstrInfo->u8NumofCh >= 1) {
		LOG_DBG("Requesting results (%u)", m2m_wifi_get_num_ap_found());
...
{
	LOG_DBG("Msg Type %d %s", message_type, wifi_cb_msg_2_str(message_type));
...
static void handle_socket_msg_connect(struct socket_data *sd, void *pvMsg)
{
	...
	LOG_ERR("CONNECT: socket %d error %d", strConnMsg->sock, strConnMsg->s8Error);

	/* Différencier les sockets net_context des sockets offload */
	if (sd->is_offload_socket) {
		LOG_DBG("CONNECT event for socket offload socket %d", strConnMsg->sock);
		/* Pour les sockets offload, pas besoin de net_context_set_state */
	} else if (sd->context) {
		/* Pour les sockets net_context */
		if (!strConnMsg->s8Error) {
			net_context_set_state(sd->context, NET_CONTEXT_CONNECTED);
		}

		if (sd->connect_cb) {
			sd->connect_cb(sd->context, strConnMsg->s8Error, sd->connect_user_data);
		}
	} else {
		LOG_WRN("CONNECT: socket %d has no context and is not marked as offload", strConnMsg->sock);
	}
...
static bool handle_socket_msg_recv(SOCKET sock, struct socket_data *sd, void *pvMsg)
{
	...
	if ((pstrRx->pu8Buffer != NULL) && (pstrRx->s16BufferSize > 0)) {
		...
		/* Différencier les sockets net_context des sockets offload */
		if (sd->is_offload_socket) {
			LOG_DBG("RECV event for socket offload socket - data handling not implemented");
			/* Socket offload devrait avoir sa propre gestion des données reçues */
		} else if (sd->recv_cb && sd->context) {
			/* Pour les sockets net_context */
			sd->recv_cb(sd->context, sd->rx_pkt, NULL, NULL, 0, sd->recv_user_data);
		} else {
			LOG_WRN("RECV: socket %d has no context and is not marked as offload", sock);
		}
...
static void handle_socket_msg_bind(struct socket_data *sd, void *pvMsg)
{
	...
	if (bind_msg->status) {
		LOG_ERR("BIND: error %d %s", bind_msg->status,
...
static void handle_socket_msg_listen(struct socket_data *sd, void *pvMsg)
{
	...
	if (listen_msg->status) {
		LOG_ERR("winc1500_socket_cb:LISTEN: error %d %s", listen_msg->status,
...
static void handle_socket_msg_accept(struct socket_data *sd, void *pvMsg)
{
	...
	LOG_DBG("ACCEPT: from %d.%d.%d.%d:%d, new socket is %d",
		accept_msg->strAddr.sin_addr.s4_addr[0], accept_msg->strAddr.sin_addr.s4_addr[1],
		accept_msg->strAddr.sin_addr.s4_addr[2], accept_msg->strAddr.sin_addr.s4_addr[3],
		ntohs(accept_msg->strAddr.sin_port), accept_msg->sock);

	if (accept_msg->sock < 0) {
		LOG_ERR("ACCEPT: error %d %s", accept_msg->sock,
	...
	/* Différencier les sockets net_context des sockets offload */
	if (sd->is_offload_socket) {
		LOG_DBG("ACCEPT event for socket offload socket - not supported");
		/* Socket offload ne supporte pas accept() pour le moment */
	} else if (sd->accept_cb && sd->context) {
		/* Pour les sockets net_context */
		...
		ret = net_context_get(AF_INET, SOCK_STREAM, IPPROTO_TCP, &a_sd->context);
		if (ret < 0) {
		 ...
		 */
		__winc1500_close((int)a_sd->context->offload_context);

		a_sd->context->offload_context = (void *)((int)accept_msg->sock);
		/** The iface is reset when getting a new context. */
		a_sd->context->iface = sd->context->iface;

		/** Setup remote */
		a_sd->context->remote.sa_family = AF_INET;
		net_sin(&a_sd->context->remote)->sin_port = accept_msg->strAddr.sin_port;
		...
		sd->accept_cb(a_sd->context, (struct sockaddr *)&accept_msg->strAddr,
			      sizeof(struct sockaddr_in),
			      (accept_msg->sock > 0) ? 0 : accept_msg->sock, sd->accept_user_data);
	} else {
		LOG_WRN("ACCEPT: socket %d has no context and is not marked as offload", accept_msg->sock);
	}
}
...
static void winc1500_socket_cb(SOCKET sock, uint8 message, void *pvMsg)
{
	...
	if (message != 6) {
		LOG_DBG("sock %d Msg %d %s", sock, message, socket_message_to_string(message));
	}
...
static int winc1500_mgmt_scan(const struct device *dev, struct wifi_scan_params *params,
			      scan_result_cb_t cb)
{
	ARG_UNUSED(params);
...
static int winc1500_mgmt_connect(const struct device *dev, struct wifi_connect_req_params *params)
{
	...
	LOG_DBG("Connecting to %s (%u) on %s %u %s security (%s)", ssid, params->ssid_length,
		channel == M2M_WIFI_CH_ALL ? "channel unknown" : "channel", channel,
		security == M2M_WIFI_SEC_OPEN ? "without" : "with",
		params->psk ? (char *)psk.au8PSK : "");

	if (m2m_wifi_connect((char *)ssid, params->ssid_length, security, auth, channel)) {
		return -EIO;
	}
...
static void winc1500_iface_init(struct net_if *iface)
{
	LOG_DBG("eth_init:net_if_set_link_addr:"
		...
		w1500_data.mac[0], w1500_data.mac[1], w1500_data.mac[2], w1500_data.mac[3],
		w1500_data.mac[4], w1500_data.mac[5]);

	net_if_set_link_addr(iface, w1500_data.mac, sizeof(w1500_data.mac), NET_LINK_ETHERNET);

	iface->if_dev->offload = &winc1500_offload;

#ifdef CONFIG_WIFI_WINC1500_SOCKET_OFFLOAD
	winc1500_socket_offload_init();
	/* Register socket offload for this interface to enable SO_BINDTODEVICE */
	net_if_socket_offload_set(iface, winc1500_socket_off_create);
#endif
...
static int winc1500_init(const struct device *dev)
{
	...
	LOG_DBG("WINC1500 MAC Address from OTP (%d) "
		...
		is_valid, w1500_data.mac[0], w1500_data.mac[1], w1500_data.mac[2],
	...
	k_thread_create(&winc1500_thread_data, winc1500_stack,
			CONFIG_WIFI_WINC1500_THREAD_STACK_SIZE, winc1500_thread, NULL, NULL, NULL,
			K_PRIO_COOP(CONFIG_WIFI_WINC1500_THREAD_PRIO), 0, K_NO_WAIT);
...
NET_DEVICE_OFFLOAD_INIT(winc1500, CONFIG_WIFI_WINC1500_NAME, winc1500_init, NULL, &w1500_data, NULL,
