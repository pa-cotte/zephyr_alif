...
void cdc200_irq_setup(const struct device *dev)
{
	...
	uint32_t linpos = (panel_cfg->vsync_len + panel_cfg->vbp +
			   panel_cfg->active_height) & 0xffff;
...
void cdc200_global_timings_set(const struct device *dev)
{
	uintptr_t regs = DEVICE_MMIO_GET(dev);
	const struct cdc200_config *config = dev->config;
	const struct cdc200_panel_config *panel_cfg =
		&config->panel_cfg;

	uint32_t sync_size = (((panel_cfg->hsync_len - 1) & 0xffff) << 16) +
			     ((panel_cfg->vsync_len - 1) & 0xffff);
	uint32_t back_porch = (((panel_cfg->hbp) & 0xffff) << 16) +
			      ((panel_cfg->vbp) & 0xffff) +
			      sync_size;
	uint32_t active_width = ((panel_cfg->active_width & 0xffff) << 16) +
				(panel_cfg->active_height & 0xffff) +
				back_porch;
	uint32_t total_width =  ((panel_cfg->hfp & 0xffff) << 16) +
				(panel_cfg->vfp & 0xffff) +
				active_width;
...
void cdc200_layer_blend_cfg_set(uintptr_t regs, enum cdc_layer idx,
		int f1, int f2)
{
	switch (idx) {
	case CDC_LAYER_1:
		sys_write32((f1 & CDC_LN_BLEND_CFG_F1_SEL_MASK) <<
			     CDC_LN_BLEND_CFG_F1_SEL_SHIFT |
...
void cdc200_layer_const_alpha_set(uintptr_t regs, enum cdc_layer idx, int alpha)
{
	switch (idx) {
	case CDC_LAYER_1:
		sys_write32(alpha & CDC_LN_CONST_ALPHA_CONSTA_MASK,
				regs + CDC_L1_CONST_ALPHA);
		break;
	case CDC_LAYER_2:
		sys_write32(alpha & CDC_LN_CONST_ALPHA_CONSTA_MASK,
				regs + CDC_L2_CONST_ALPHA);
		break;
	default:
		LOG_ERR("Incorrect Layer ID.");
	}
...
int cdc200_layer_fb_setup(const struct device *dev, enum cdc_layer idx)
{
	...
	case CDC_LAYER_1:
		...
		sys_write32(((fb_len & CDC_LN_CFB_LENGTH_PITCH_MASK) <<
					CDC_LN_CFB_LENGTH_PITCH_SHIFT) |
			    (fb_llen & CDC_LN_CFB_LENGTH_LLENGTH_MASK),
				regs + CDC_L1_CFB_LENGTH);
		sys_write32(layer->y1 - layer->y0,
				regs + CDC_L1_CFB_LINES);
		break;
	case CDC_LAYER_2:
...
int cdc200_layer_dflt_color_set(const struct device *dev, enum cdc_layer idx)
{
	 ...
	 */
	if (!layer->def_color_en ||
	    (layer->layer_en &&
	    (!(lw - panel_cfg->active_width) &&
	...
	case CDC_LAYER_1:
		...
		sys_write32(temp, regs + CDC_L1_CTRL);
		sys_write32(layer->def_color,
				regs + CDC_L1_DFLT_COLOR);
		break;
	case CDC_LAYER_2:
		temp = sys_read32(regs + CDC_L2_CTRL);
		temp |= CDC_LN_CTRL_DFLT_CBLEND_EN;
...
void cdc200_layer_blending_setup(const struct device *dev, enum cdc_layer idx)
{
	...
	case CDC_PIXEL_FORMAT_RGB565:
		cdc200_layer_blend_cfg_set(regs,
				idx,
				CDC_BLEND_CONST_ALPHA,
				CDC_BLEND_CONST_ALPHA_INV);
		break;
	default:
		cdc200_layer_blend_cfg_set(regs,
				idx,
				layer->curr_layer_bf_f1,
...
int cdc200_layer_window_set(const struct device *dev, enum cdc_layer idx)
{
	...
	uint32_t layer_hstart =
		panel_cfg->hsync_len + panel_cfg->hbp + layer->x0;
	uint32_t layer_hend =
		panel_cfg->hsync_len + panel_cfg->hbp + layer->x1 - 1;
	uint32_t layer_vstart =
		panel_cfg->vsync_len + panel_cfg->vbp + layer->y0;
	uint32_t layer_vend =
		panel_cfg->vsync_len + panel_cfg->vbp + layer->y1 - 1;

	if ((layer->x1 > panel_cfg->active_width) ||
	    (layer->y1 > panel_cfg->active_height)) {
...
int cdc200_setup_registers(const struct device *dev)
{
	...
	for (int i = CDC_LAYER_1; i <= CDC_LAYER_2; i++) {
		...
		if (layer[i].layer_en) {
			...
			if (ret) {
				LOG_ERR("Failed setting window on layer - %d",
						(i + 1));
				return ret;
			}
...
static int cdc200_set_brightness(const struct device *dev,
				 const uint8_t brightness)
{
	return -ENOTSUP;
}

static int cdc200_set_contrast(const struct device *dev,
		const uint8_t contrast)
{
	return -ENOTSUP;
}

void cdc200_generic_get_caps(const struct device *dev,
...
static int cdc200_set_pixel_format(const struct device *dev,
			...
			struct cdc200_display_caps *capabilities)
{
	...
	for (int i = CDC_LAYER_1; i <= CDC_LAYER_2; i++) {
		...
		capabilities->layer[i].x_resolution =
			layer[i].x1 - layer[i].x0;
		capabilities->layer[i].y_resolution =
			layer[i].y1 - layer[i].y0;
		switch (layer[i].fmt) {
		case CDC_PIXEL_FORMAT_ARGB8888:
...
static void cdc200_isr(const struct device *dev)
{
	...
	if (irq_st & CDC_IRQ_LINE) {
		...
		if (data->curr_fb[CDC_LAYER_1] != data->next_fb[CDC_LAYER_1]) {
			sys_write32((uint32_t)data->next_fb[CDC_LAYER_1],
				regs + CDC_L1_CFB_ADDR);
			data->curr_fb[CDC_LAYER_1] = data->next_fb[CDC_LAYER_1];
		}

		if (data->curr_fb[CDC_LAYER_2] != data->next_fb[CDC_LAYER_2]) {
			sys_write32((uint32_t)data->next_fb[CDC_LAYER_2],
				regs + CDC_L2_CFB_ADDR);
...
static int cdc200_init(const struct device *dev)
{
	...
	for (uint32_t i = CDC_LAYER_1; i < CDC_LAYER_MAX; i++) {
		...
		LOG_DBG("\tLayer %d FB-Addr - 0x%x", (i + 1),
				(uint32_t)layer->fb_addr);
		LOG_DBG("\tLayer %d FB size - %d", (i + 1), layer->fb_size);
		LOG_DBG("\tLayer %d FB Format - %d", (i + 1), layer->fmt);

		LOG_DBG("\tLayer %d windowing (x0, y0) - (%d, %d)",
				(i + 1), layer->x0, layer->y0);
		LOG_DBG("\tLayer %d windowing (x1, y1) - (%d, %d)",
				(i + 1), layer->x1, layer->y1);
	}
	return 0;
}
...
#define ALLOCATE_FB0(i)										\
	IF_ENABLED(DT_INST_PROP(i, enable_l1),							\
		(static uint8_t FRAME_BUFFER_SECTION fb0_##i[CDC200_PIXEL_SIZE_LAYER(i, 1) *	\
			(COND_CODE_1(DT_INST_NODE_HAS_PROP(i, win_x1_l1),			\
				     (DT_INST_PROP(i, win_x1_l1)),				\
				     (DT_INST_PROP(i, width))) - DT_INST_PROP(i, win_x0_l1)) *	\
			(COND_CODE_1(DT_INST_NODE_HAS_PROP(i, win_y1_l1),			\
				     (DT_INST_PROP(i, win_y1_l1)),				\
				     (DT_INST_PROP(i, height))) - DT_INST_PROP(i, win_y0_l1))]))

#define ALLOCATE_FB1(i)										\
	IF_ENABLED(DT_INST_PROP(i, enable_l2),							\
		(static uint8_t FRAME_BUFFER_SECTION fb1_##i[CDC200_PIXEL_SIZE_LAYER(i, 2) *	\
			(COND_CODE_1(DT_INST_NODE_HAS_PROP(i, win_x1_l2),			\
				     (DT_INST_PROP(i, win_x1_l2)),				\
				     (DT_INST_PROP(i, width))) - DT_INST_PROP(i, win_x0_l2)) *	\
			(COND_CODE_1(DT_INST_NODE_HAS_PROP(i, win_y1_l2),			\
				     (DT_INST_PROP(i, win_y1_l2)),				\
				     (DT_INST_PROP(i, height))) - DT_INST_PROP(i, win_y0_l2))]))
#define FB0(i)	\
	COND_CODE_1(DT_INST_PROP(i, enable_l1), (fb0_##i), (NULL))

#define FB1(i)	\
	COND_CODE_1(DT_INST_PROP(i, enable_l2), (fb1_##i), (NULL))
...
#define CDC200_DEFINE(i)									\
	...
	static const struct cdc200_config config##i = {						\
		...
		.layer[0] = {									\
			...
			.fb_addr = FB0(i),							\
			.fb_size = FB0_SIZE(i),							\
			.fmt = DT_INST_ENUM_IDX_OR(i, pixel_fmt_l1, CDC_PIXEL_FORMAT_UNDEFINED),\
			.pixel_size = CDC200_PIXEL_SIZE_LAYER(i, 1),				\
			.def_color_en = DT_INST_NODE_HAS_PROP(i, def_back_color_l1),		\
			.def_color = COND_CODE_1(DT_INST_NODE_HAS_PROP(i, def_back_color_l1),	\
						 (DT_INST_PROP(i, def_back_color_l1)),		\
						 (0)),						\
		...
		.layer[1] = {									\
			...
			.fb_addr = FB1(i),							\
			.fb_size = FB1_SIZE(i),							\
			.fmt = DT_INST_ENUM_IDX_OR(i, pixel_fmt_l2, CDC_PIXEL_FORMAT_UNDEFINED),\
			.pixel_size = CDC200_PIXEL_SIZE_LAYER(i, 2),				\
			.def_color_en = DT_INST_NODE_HAS_PROP(i, def_back_color_l2),		\
			.def_color = COND_CODE_1(DT_INST_NODE_HAS_PROP(i, def_back_color_l2),	\
						 (DT_INST_PROP(i, def_back_color_l2)),		\
						 (0)),						\
	...
	{											\
		IRQ_CONNECT(DT_INST_IRQ_BY_NAME(i, scanline_0, irq),				\
			DT_INST_IRQ_BY_NAME(i, scanline_0, priority),				\
			cdc200_isr,								\
