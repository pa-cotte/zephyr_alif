...
static const struct dphy_pll_vco_ctrl vco_ctrl_range[] = {
	{ MHZ(1170), MHZ(1250), 0x03 },
	{ MHZ(975), MHZ(1230), 0x07 },
	{ MHZ(853.125), MHZ(1025.0), 0x08 },
	{ MHZ(706.875), MHZ(896.875), 0x08 },
	{ MHZ(585), MHZ(743.125), 0x0b },
	{ MHZ(487.5), MHZ(615), 0x0f },
	{ MHZ(426.56), MHZ(512.5), 0x10 },
	{ MHZ(353.4), MHZ(484.4), 0x10 },
	{ MHZ(292.5), MHZ(371.5), 0x13 },
	{ MHZ(243.75), MHZ(307.5), 0x17 },
	{ MHZ(213.3), MHZ(256.25), 0x18 },
	{ MHZ(176.72), MHZ(224.2), 0x18 },
	{ MHZ(146.25), MHZ(185.78), 0x1b },
	{ MHZ(121.88), MHZ(153.75), 0x1f },
	{ MHZ(106.64), MHZ(125.12), 0x20 },
	{ MHZ(88.36), MHZ(112.1), 0x20 },
	{ MHZ(73.13), MHZ(92.9), 0x23},
	{ MHZ(60.93), MHZ(76.87), 0x27 },
	{ MHZ(53.32), MHZ(64), 0x28 },
	{ MHZ(44.18), MHZ(56), 0x28 },
...
static uint8_t mipi_dphy_read_register(uintptr_t test_ctrl0,
		uintptr_t test_ctrl1,
		uint16_t address)
{
	uint8_t ret = 0;
	...
	reg_write_part(test_ctrl1,
			0x0,
			PHY_TST_CTRL1_TESTDIN_MASK,
...
static void mipi_dphy_write_register(uintptr_t test_ctrl0,
		uintptr_t test_ctrl1,
		uint16_t address,
		uint8_t data)
{
...
void mipi_dphy_mask_write_register(uintptr_t test_ctrl0,
		uintptr_t test_ctrl1,
		uint16_t address,
		uint8_t data,
		uint8_t mask,
...
static int dphy_pll_calculate_mnp(const struct ensemble_dphy_config *config,
		uint32_t pll_fout, uint16_t *pll_m,
		uint16_t *pll_n, uint8_t *pll_p, uint8_t *vco_ctrl)
{
	uint32_t delta = -1;
	...
	for (int i = 0; i < ARRAY_SIZE(vco_ctrl_range); i++) {
		...
		if ((config->ref_frequency/n) > MHZ(24) ||
		   (config->ref_frequency/n) < MHZ(8))
			continue;

		m = CEIL((freq_mhz * 2 * p * n) /
			(config->ref_frequency/1000000.0f));

		tmp = config->ref_frequency / (n * (p<<1));
		tmp *= m;

		calculated_freq = (uint32_t) tmp;
		if (abs(pll_fout - calculated_freq) < delta) {
			delta = abs(pll_fout - calculated_freq);
			*pll_m = m;
			*pll_n = n;
			*pll_p = p;
			*vco_ctrl = vco;
...
static int dphy_config_pll(const struct device *dev,
		struct dphy_dsi_settings *phy)
{
	 ...
	 */
	ret = dphy_pll_calculate_mnp(config, phy->pll_fout, &pll_m, &pll_n, &pll_p,
			&vco_cntrl);
	if (ret) {
	 ...
	 */
	reg_write_part(regs + DPHY_PLL_CTRL0,
			1,
			DPHY_PLL_CTRL0_CLKSEL_MASK,
			DPHY_PLL_CTRL0_CLKSEL_SHIFT);

	/* Set PLL m[9:0] to regs - 0x179, 0x17a */
	mipi_dphy_mask_write_register(test_ctrl0,
			test_ctrl1,
	...
	tmp = mipi_dphy_read_register(test_ctrl0,
			test_ctrl1,
	...
	tmp |= BIT(7);
	mipi_dphy_write_register(test_ctrl0,
			test_ctrl1,
			dphy4txtester_DIG_RDWR_TX_PLL_30,
			tmp);
	...
	mipi_dphy_mask_write_register(test_ctrl0,
			test_ctrl1,
			dphy4txtester_DIG_RDWR_TX_CB_3,
			0x02,
			0x7,
			0);
		...
		struct dphy_dsi_settings *phy)
{
	...
	for (i = 0; ((i < ARRAY_SIZE(frequency_range) - 1) &&
		(bitrate_mbps > frequency_range[i].bitrate_in_mbps)); i++) {
	}
	...
	LOG_DBG("CLK HS2LP: %d, CLK LP2HS: %d, LANE_HS2LP: %d, LANE_LP2HS: %d",
			phy->clk_hs2lp, phy->clk_lp2hs,
			phy->lane_hs2lp, phy->lane_lp2hs);

	/* Setup number of lanes. */
	reg_write_part(dsi_regs + DSI_PHY_IF_CFG,
			phy->num_lanes - 1,
			DSI_PHY_IF_CFG_PHY_N_LANES_MASK,
			DSI_PHY_IF_CFG_PHY_N_LANES_SHIFT);
	...
	reg_write_part(regs + TX_DPHY_CTRL0,
			hsfreq,
			DPHY_CTRL0_HS_FREQ_RANGE_MASK,
	...
	mipi_dphy_write_register(test_ctrl0,
			test_ctrl1,
			dphy4txtester_DIG_RDWR_TX_CLK_TERMLOWCAP,
			0x2);
	...
	if (phy->num_lanes == 1) {
		mipi_dphy_write_register(test_ctrl0,
				test_ctrl1,
				dphy4txtester_DIG_RDWR_TX_LANE1_SLEWRATE_0,
				0xe);
	}
	mipi_dphy_write_register(test_ctrl0,
			test_ctrl1,
	...
	mipi_dphy_write_register(test_ctrl0,
			test_ctrl1,
			dphy4txtester_DIG_RDWR_TX_LANE3_SLEWRATE_0,
			0xe);

	cfgclkfreqrange = ((config->cfg_clk_frequency / MHZ(1)) - 17) << 2;
	reg_write_part(regs + TX_DPHY_CTRL0,
			cfgclkfreqrange,
			DPHY_CTRL0_CFG_CLK_FREQ_RANGE_MASK,
	...
	sys_clear_bits(regs + TX_DPHY_CTRL1,
			tmp << DPHY_CTRL1_FORCE_RX_MODE_SHIFT);
	...
	for (int i = 0; (i < 1000000) &&
		(sys_read32(dsi_regs + DSI_PHY_STATUS) & tmp) != tmp; i++) {
		k_busy_wait(1);
	}
		...
		struct dphy_csi2_settings *phy)
{
	...
	sys_clear_bits(csi_regs + CSI_PHY_SHUTDOWNZ,
		CSI_PHY_SHUTDOWNZ_PHY_SHUTDOWNZ);
	...
	reg_write_part(regs + RX_DPHY_CTRL0,
			hsfreq,
			DPHY_CTRL0_HS_FREQ_RANGE_MASK,
			DPHY_CTRL0_HS_FREQ_RANGE_SHIFT);
	...
	sys_set_bits(regs + RX_DPHY_CTRL1,
		tmp << DPHY_CTRL1_FORCE_RX_MODE_SHIFT);
	...
	tmp = (phy->num_lanes == 2) ?
		(tmp | CSI_PHY_STOPSTATE_PHY_STOPSTATEDATA_1) : tmp;
	...
	tmp = (1U << phy->num_lanes) - 1;
	sys_clear_bits(regs + RX_DPHY_CTRL1,
		tmp << DPHY_CTRL1_FORCE_RX_MODE_SHIFT);
...
static int ensemble_dphy_init(const struct device *dev)
{
	...
	LOG_DBG("Config Clk: %d Ref Clk: %d", config->cfg_clk_frequency,
...
#define ALIF_MIPI_DPHY_DEVICE(i)						\
	...
	DEVICE_DT_INST_DEFINE(i,						\
			...
			&config_##i,						\
			POST_KERNEL,						\
			CONFIG_MIPI_DPHY_INIT_PRIORITY,				\
			NULL);
